...
let heartRate3D : Double = 70
let addedHRD = heartRate1D + heartRate2D + heartRate3D
let averageHRD = addedHRD / 3.0
print("Average Heart Rate: \(averageHRD)")
//Yes, it differs from the previous result because in the second example we are working with Double values, which allow decimal points. And in the first example we are working with integers, which truncates any decimal part.
